...
# ================== ADD COINS AND CHECK COINS ==================

async def add_coins(update: Update, context: CallbackContext):
    """Handle /add_coins command to add coins to a user"""
    if update.message.from_user.id not in ADMIN_IDS:
        return

    if len(context.args) != 2:
        await update.message.reply_text("⚠️ Please provide the user ID and the number of coins.")
        return
    ...
    try:
        ...
        if user_str_id not in data["users"]:
            data["users"][user_str_id] = {"coins": 0}
        
        data["users"][user_str_id]["coins"] += coins_to_add

        save_user_data(data)
        await update.message.reply_text(f"✅ Added {coins_to_add} coins to user {user_id}.")
    
    except ValueError:
        await update.message.reply_text("⚠️ Invalid input. Please make sure both user ID and coins are numbers.")


async def check_coins(update: Update, context: CallbackContext):
    """Handle /check_coins command to check the user's coins"""
    if len(context.args) != 1:
        await update.message.reply_text("⚠️ Please provide the user ID to check the coins.")
        return
    ...
    try:
        ...
        if user_str_id in data["users"]:
            coins = data["users"][user_str_id].get("coins", 0)
            await update.message.reply_text(f"💰 User {user_id} has {coins} coins.")
        else:
            await update.message.reply_text(f"⚠️ User {user_id} not found.")
    
    except ValueError:
        await update.message.reply_text("⚠️ Invalid input. Please make sure the user ID is a number.")
...
def main():
    ...
    handlers = [
        ...
        CommandHandler("add_coins", add_coins),  # Now the add_coins function is defined
        CommandHandler("check_coins", check_coins),  # Now the check_coins function is defined
